		...
		<>
			{img.map((src, i) => (
				<img
					title={title[i]}
					key={i}
					className="icons hover-effect m-1"
					height="50"
					src={src}
				/>
			))}
...
function SkillsImg({img, title}) {
	return (
		<>
			<div className="col-md-6 text-center">
				<AddImgs img={img} title={title} />
			</div>
		</>
	);
}
			...
			<div className={`timeline-card timeline-card-${color} card shadow-sm`}>
				<div className="card-body">
					<div className="h5 mb-1">
						{job} <span className="text-muted h6">{company}</span>
					</div>
